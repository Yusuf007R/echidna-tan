...
player.post('/volume/:volume', (req, res) => {
  ...
  if (volume > 100 || volume < 0) throw new CustomException('Volume must be between 0 and 100', 400);
...
player.post('/loop/:loopMode', (req, res) => {
  ...
  if (!LoopState[loopMode]) throw new CustomException('Loop mode must be none, all or single', 400);
