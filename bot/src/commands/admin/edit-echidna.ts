...
export default class EditEchidnaCommand extends Command<typeof options> {
  ...
  async run(interaction: ChatInputCommandInteraction) {
    ...
    if (avatar) {
      if (!['image/jpeg', 'image/png', 'image/jpg'].includes(avatar.contentType ?? '')) {
    ...
    let dbBody: Partial<InferInsertModel<typeof echidnaTable>> = {};

    if (activityType) {
      const activityTypeValue = activityTypeValues.find(({ name }) => name === activityType)?.value;
