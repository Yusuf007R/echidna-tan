...
class GetChoices {
  ...
  private get(key: string, required = false) {
    ...
    if (required) {
      ...
      if (!option.value && option.value !== 0) throw new Error(`Missing value for option ${key}`);
  ...
  getString<B extends boolean>(key: string, required = false as B) {
    ...
    if (option !== undefined && typeof option !== 'string') throw new Error(`Option ${key} is not a string`);
    return option as R<string, B>;
  }

  getNumber<B extends boolean>(key: string, required = false as B) {
    const option = this.get(key, required);
    if (option !== undefined && typeof option !== 'number') throw new Error(`Option ${key} is not a number`);
