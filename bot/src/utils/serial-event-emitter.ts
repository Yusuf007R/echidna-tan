...
class SerialEventEmitter<EventMap extends DefaultEventMap = DefaultEventMap> extends EventEmitter<EventMap> {
  ...
  queueSize(eventName: keyof EventMap): number {
    return this.queue.has(eventName) ? this.queue.get(eventName)!.length : 0;
  }
