...
export type Option = StringOption | BoolOption | IntOption | UserOption | AttachmentOption | SubCommandOption;
...
export class OptionsBuilder<const T extends Option[] = []> {
  ...
  addSubCommandOption<const O extends Omit<SubCommandOption, 'type' | 'options'>, TO extends Option[] = []>(
    config: O,
    ...
    this.options.push({ type: 'sub-command', ...config, options: subCommandOptions });
    return this as any;
  }
