    ...
    .input(
      z.union([
        ...
        z.object({
          ...
          input: z.union([z.literal('NONE'), z.literal('TRACK'), z.literal('QUEUE')])
        })
      ])
    )
    .mutation(({ input }) => {
      return EchidnaSingleton.echidna.musicPlayer.get('').callMethod(input.name, input?.input);
    }),
  data: guildProcedure.subscription(({ ctx }) => {
    return observable<playerData>((emit) => {
      const methodCallFN = (data: { method: string; guild: string }) => {
        ...
        emit.next(EchidnaSingleton.echidna.musicPlayer.get(guild).getPlayerData());
      };
      EchidnaSingleton.echidna.musicPlayer.methodEmitter.on('methodCalled', methodCallFN);
      return () => {
        EchidnaSingleton.echidna.musicPlayer.methodEmitter.off('methodCalled', methodCallFN);
      };
    });
  })
});
